...
class Model():
    ...
    def _init_placeholder(self):
        ...
        #tf.abs求绝对值
        #tf.sign求返回符号 y = sign(x) = -1 if x < 0; 0 if x == 0; 1 if x > 0
        #tf.reduce_sum计算输入tensor元素的和，或者安照reduction_indices指定的轴进行求和
        #tf.cast将x或者x.values转换为dtype
        #tf.shape返回数据的shape
    ...
    def embedding_layer(self):
        #tf.variable_scope会生成一个上下文管理器，并明确指定在这个上下文管理器中，tf.get_variable将直接获得已经生成的变量
        with tf.variable_scope("embedding_layer") as scope:
            ...
            #tf.nn.embedding_lookup（tensor, id）:tensor就是输入张量，id就是张量对应的索引,主要是选取一个张量里面索引对应的元素
            ...
            for direction in ["forward", "backward"]:
                with tf.variable_scope(direction):
                    lstm_cell[direction] = rnn.GRUCell(
                        ...
                        # use_peepholes=True,:q
                        # initializer=self.initializer,
                        # state_is_tuple=True
                    )

            #使用tensorflow.nn.bidirectional_dynamic_rnn()这个函数，就可以很方便的实现双向LSTM
    ...
    def loss_layer(self):
        with tf.variable_scope("loss_layer"):
            ...
            start_logits = tf.concat(
                [small * tf.ones(shape=[self.batch_size, 1, self.nums_tags]), tf.zeros(shape=[self.batch_size, 1, 1])], axis=-1
            )
            pad_logits = tf.cast(
                small * tf.ones([self.batch_size, self.nums_steps, 1]), tf.float32
            )
            ...
            targets = tf.concat(
                [tf.cast(self.nums_tags*tf.ones([self.batch_size, 1]), tf.int32),self.targets], axis=-1
            )
            logits = self.logits
            ...
            #crf_log_likelihood函数的目的：使用crf 来计算损失，里面用到的优化方法是：最大似然估计,在一个条件随机场里面计算标签序列的log-likelihood
    ...
    def optimizer_layer(self):
        with tf.variable_scope("optimizer"):
            self.train_op = tf.train.AdamOptimizer().minimize(self.loss, global_step=self.global_steps)
        #tf.global_variables()获取全部变量
    ...
    def step(self, sess, batch):
        ...
        global_steps, loss, _, logits, acc, length = sess.run([self.global_steps, self.loss, self.train_op, self.logits, self.accuracy, self.length], feed_dict=feed)
        ...
        with tf.Session() as sess:
            ...
            for i in range(self.max_epoch):
                ...
                for batch in self.train_data.get_batch():
                    steps += 1
                    global_steps, loss, logits, acc, length = self.step(sess, batch)
                    if steps % 10 == 0:
                        print "[->] step {}/{}\tloss {:.2f}\tacc {:.2f}".format(steps, len(self.train_data.batch_data), loss, acc)
    ...
    def evaluate(self, sess, tag):
        ...
        # _,_,scores,_,lengths = self.step(sess, batch)
        pre_paths = self.decode(scores, lengths, trans)
        tar_paths = [i[1] for i in batch]
    ...
    def prepare_pred_data(self, text):
        ...
        vec = [self.vocab.get(i.encode("utf-8"), 0) for i in text]
        feed = {
            self.inputs:[vec],
            self.dropout:1
        }
    ...
    def predict(self):
        ...
        with tf.Session() as sess:
            ...
            while True:
                ...
                logits,length = sess.run([self.logits, self.length], feed_dict=feed)
                ...
                resp = {
                    "ORG":org_entity["entities"],
                    "PER":per_entity["entities"]
                }
